  ...
  const getTechnologyCharacteristics = useMemo(() => () => {
    ...
    const profileSettings = {
      conservative: {
        ...
        forceInclude: ['equity', 'transparency', 'human_oversight'],
  ...
  const getGroupedRequirements = () => {
    ...
    selectedChars.forEach(char => {
      ...
      if (status === 'evaluating') {
        groups['Documentation Requirements'].push(`Evaluation protocol for ${char.name}`);
        groups['Monitoring & Reporting'].push(`Metrics collection for ${char.name}`);
        groups['Quality Assurance'].push(`Performance benchmarks for ${char.name}`);
      } else if (status === 'no_risk') {
        groups['Documentation Requirements'].push(`Risk assessment justification for ${char.name}`);
        groups['Review & Approval'].push(`Committee review of ${char.name} assessment`);
      } else if (status === 'mitigated') {
        groups['Technical Controls'].push(`Validated controls for ${char.name}`);
        groups['Quality Assurance'].push(`Control effectiveness monitoring for ${char.name}`);
      }
    });
